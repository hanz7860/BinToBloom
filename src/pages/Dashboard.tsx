...
import { fetchUserPickups } from "../store/slices/pickupSlice"
import { fetchNotifications } from "../store/slices/notificationSlice"
import { Link } from "react-router-dom"
import { Package, Clock, CheckCircle, Truck, Award, TrendingUp, MapPin } from "lucide-react"
              ...
              <div className="divide-y divide-gray-200">
                {pickups.length === 0 ? (
                  <div className="p-6 text-center text-gray-500">
                    ...
                    {user?.role === "DONOR" && (
                      <Link to="/schedule-pickup" className="text-green-600 hover:text-green-500 font-medium">
                    ...
                    <div key={pickup.id} className="p-6">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-4">
                          <div className="flex-shrink-0">{getStatusIcon(pickup.status)}</div>
                          <div>
                            ...
                            <p className="text-xs text-gray-400 mt-1">
                              Scheduled: {formatDistanceToNow(new Date(pickup.scheduledTime), { addSuffix: true })}
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="mt-3 p-3 bg-gray-50 rounded-lg">
                          ...
                          {pickup.status === "ACCEPTED" && (
                            <p className="text-sm text-green-600 mt-1">
                              ✓ Your pickup has been accepted and collector is on the way!
                            </p>
                          )}
                          ...
                          {pickup.status === "COMPLETED" && (
                            <p className="text-sm text-green-600 mt-1">✅ Pickup completed successfully!</p>
                          )}
                        </div>
                      )}
                    </div>
              ...
              <div className="p-6">
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm font-medium text-green-600">{(stats.totalWaste * 0.28).toFixed(1)}kg</span>
                  </div>
                  <div className="flex justify-between items-center">
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-gray-600">Pesticide Produced</span>
                    <span className="text-sm font-medium text-green-600">{(stats.totalWaste * 0.6).toFixed(1)}L</span>
                  </div>
                </div>
                ...
                <div className="mt-6 p-4 bg-green-50 rounded-lg">
                  <div className="flex items-center">
                    ...
                    <span className="text-sm font-medium text-green-800">Eco Warrior Badge</span>
                  </div>
