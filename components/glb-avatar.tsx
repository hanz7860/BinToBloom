...
export function GLBAvatar({
  ...
  const [animationState, setAnimationState] = useState<
    'idle' | 'wave' | 'dance' | 'thinking'
  >('idle')
  const [loadError, setLoadError] = useState(false)

  // Use drei's useGLTF hook instead of useLoader with GLTFLoader directly
  const { scene, animations } = useGLTF(modelPath, true, undefined, (error) => {
  ...
  useEffect(() => {
    if (actions && Object.keys(actions).length > 0) {
      ...
      switch (animationState) {
        case 'idle':
          if (actions.idle || actions.Idle || animationNames[0]) {
            const idleAction =
              actions.idle || actions.Idle || actions[animationNames[0]]
            idleAction?.play()
          }
          break
        case 'wave':
          if (actions.wave || actions.Wave) {
            const waveAction = actions.wave || actions.Wave
            waveAction?.play()
          }
          break
        case 'dance':
          if (actions.dance || actions.Dance) {
            const danceAction = actions.dance || actions.Dance
            danceAction?.play()
          }
          break
        case 'thinking':
          if (actions.thinking || actions.Thinking) {
            const thinkAction = actions.thinking || actions.Thinking
            thinkAction?.play()
          }
          break
      }
  ...
  return (
    ...
    >
      {/* Avatar - either GLB model or placeholder */}
      {!loadError && scene ? (
        <primitive
          object={clone(scene)}
          scale={2}
  ...
  return (
    <group position={[0, -1, 0]} scale={1.5}>
      ...
      <mesh position={[0, 2.1, -0.1]} castShadow>
        <sphereGeometry
          args={[0.38, 16, 16, 0, Math.PI * 2, 0, Math.PI * 0.7]}
        />
      ...
      <Html position={[0, 0.5, 0.5]} center>
        <div
          className="bg-white px-2 py-1 rounded text-xs font-bold shadow-lg border"
          style={{ color }}
        >
          {name[0]}
        </div>
...
// Pre-load models to avoid waterfall loading
useGLTF.preload('/models/aniket.glb')
useGLTF.preload('/models/harsh.glb')
useGLTF.preload('/models/hanzala.glb')
useGLTF.preload('/models/fardeen.glb')
useGLTF.preload('/models/maroti.glb')
